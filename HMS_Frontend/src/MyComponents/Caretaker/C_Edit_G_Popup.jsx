...
import { add_G_Emp, edit_G_Emp } from '../../endpoints/endpoint';
import "../../css/Warden/W_Add_St_Popup.css"

export const C_Edit_G_Popup = ({childToParent, editGd}) => {

    const [name, setName ] = React.useState(editGd.name);
    const [phNo, setPhNo ] = React.useState(editGd.phNo);
    const [jobId, setjobId ] = React.useState(editGd.jobId);
    const [address, setAddress ] = React.useState(editGd.address);
    const [password, setPassword ] = React.useState("");
    const [onDuty, setOnDuty ] = React.useState(false);
    const [email, setEmail ] = React.useState(editGd.email);
    ...
    const editGEmp = async() =>{
        ...
        response = await edit_G_Emp(message)
        console.log(response);
        childToParent('close g popup');
    }
